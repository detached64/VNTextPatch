...
namespace VNTextPatch.Scripts.Yuris
{
    public class YurisScenarioScript : IScript
    {
        ...
        public IEnumerable<ScriptString> GetStrings()
        {
            ...
            {
                string str = string.Join(string.Empty, attrs.Select(a => GetAttributeValue(a)));
        ...
        private IEnumerable<List<YurisAttribute>> GetStringAttributeGroups()
        {
            ...
            foreach (YurisCommand command in GetCommands())
            {
                ...
                if (command.Id == _wordCommandId ||
                    ...
                    command.Id == _evalCommandId && !(GetAttributeValue(commandAttrs[0]) ?? string.Empty).Contains("\\p"))
                {
